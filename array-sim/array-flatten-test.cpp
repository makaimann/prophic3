...
int main() {
  ...
  msat_term trans = msat_make_eq(
      env, outN,
      msat_make_bv_plus(env, out, msat_make_bv_int_number(env, 2, 4)));
  trans = msat_make_and(
      env, trans,
      msat_make_eq(
          env, dataN,
  ...
  trans = msat_make_and(
      ...
      msat_make_eq(
          ...
          msat_make_array_write(
              env, msat_make_array_write(env, arr, out, data),
              msat_make_bv_plus(env, out, msat_make_bv_int_number(env, 1, 4)),
              data)));
  msat_term prop =
      msat_make_bv_ult(env, out, msat_make_bv_int_number(env, 10, 4));
