...
Options get_options(int argc, const char **argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "-v") {
        ...
        } else if (a == "-no-proph-var-reduction") {
          ret.proph_var_reduction = false;
          ...
          std::cout
              << "USAGE: " << argv[0] << " [OPTIONS] FILENAME.vmt"
              ...
              << "\n   -no-axiom-reduction: don't reduce enumerated "
                 "axioms at all"
              << "\n   -no-proph-var-reduction: don't attempt to limit "
                 "number of prophecy variables"
              ...
              << "\n   -assume-prop: assume property in trans (default: off)"
              << std::endl;
          exit(0);
          break;
