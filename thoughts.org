...
** Is witness part of index set?
*** In our current code, no
*** In Aaron Bradley formulation it would be I think
*** For us, I'm not sure it needs to be -- only used for comparisons between arrays
** Optimizations
*** Pruning lemmas
*** Inputs vs state
