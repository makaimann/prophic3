(set-info :source |printed by MathSAT|)
(declare-fun C_3 () Int)
(declare-fun .y.10 () Int)
(declare-fun E_3 () Int)
(declare-fun .y.11 () Int)
(declare-fun F_3 () Int)
(declare-fun .y.12 () Int)
(declare-fun v_6 () Int)
(declare-fun .y.13 () Int)
(declare-fun A_3 () Int)
(declare-fun .y.14 () Int)
(declare-fun .loc.8 () Bool)
(declare-fun .loc.15 () Bool)
(declare-fun .loc.9 () Bool)
(declare-fun .loc.16 () Bool)
(declare-fun B_3 () (Array Int Int))
(declare-fun B () (Array Int Int))
(define-fun .def_8 () Int (! C_3 :next |.y.10|))
(define-fun .def_10 () Int (! E_3 :next |.y.11|))
(define-fun .def_12 () Int (! F_3 :next |.y.12|))
(define-fun .def_14 () Int (! v_6 :next |.y.13|))
(define-fun .def_16 () Int (! A_3 :next |.y.14|))
(define-fun .def_18 () Bool (! .loc.8 :next |.loc.15|))
(define-fun .def_20 () Bool (! .loc.9 :next |.loc.16|))
(define-fun .def_36 () (Array Int Int) (! B_3 :next |.xtv.1|))
(define-fun .def_86 () (Array Int Int) (! B :next |.xiv.1|))
(define-fun .def_84 () Bool (not .def_20))
(define-fun .def_28 () Bool (not .def_18))
(define-fun .def_91 () Bool (and .def_28 .def_84))
(define-fun .def_89 () Bool (= .def_8 0))
(define-fun .def_87 () Int (select .def_86 .def_14))
(define-fun .def_88 () Bool (= .def_16 .def_87))
(define-fun .def_90 () Bool (and .def_88 .def_89))
(define-fun .def_92 () Bool (! (and .def_90 .def_91) :init true))
(define-fun .def_70 () Int (select .def_36 .def_8))
(define-fun .def_75 () Bool (= .def_10 .def_70))
(define-fun .def_76 () Bool (not .def_75))
(define-fun .def_72 () Bool (= .def_8 .def_14))
(define-fun .def_71 () Bool (= .def_16 .def_70))
(define-fun .def_73 () Bool (and .def_71 .def_72))
(define-fun .def_23 () Bool (<= .def_12 .def_8))
(define-fun .def_24 () Bool (not .def_23))
(define-fun .def_74 () Bool (and .def_24 .def_73))
(define-fun .def_77 () Bool (and .def_74 .def_76))
(define-fun .def_78 () Bool (and .def_18 .def_77))
(define-fun .def_79 () Bool (and .loc.16 .def_78))
(define-fun .def_22 () Bool (not .loc.15))
(define-fun .def_80 () Bool (and .def_22 .def_79))
(define-fun .def_68 () Bool (= .y.10 0))
(define-fun .def_59 () Bool (and .loc.15 .def_23))
(define-fun .def_26 () Bool (not .loc.16))
(define-fun .def_60 () Bool (and .def_26 .def_59))
(define-fun .def_61 () Bool (and .def_28 .def_60))
(define-fun .def_30 () Bool (= .def_14 .y.13))
(define-fun .def_62 () Bool (and .def_30 .def_61))
(define-fun .def_32 () Bool (= .def_12 .y.12))
(define-fun .def_63 () Bool (and .def_32 .def_62))
(define-fun .def_34 () Bool (= .def_10 .y.11))
(define-fun .def_64 () Bool (and .def_34 .def_63))
(define-fun .def_37 () Int (select .def_36 .def_14))
(define-fun .def_55 () Bool (= .y.14 .def_37))
(define-fun .def_65 () Bool (and .def_55 .def_64))
(define-fun .def_38 () Bool (= .def_16 .def_37))
(define-fun .def_66 () Bool (and .def_38 .def_65))
(define-fun .def_69 () Bool (and .def_66 .def_68))
(define-fun .def_81 () Bool (or .def_69 .def_80))
(define-fun .def_49 () Bool (and .def_18 .def_24))
(define-fun .def_50 () Bool (and .loc.15 .def_49))
(define-fun .def_51 () Bool (and .def_26 .def_50))
(define-fun .def_52 () Bool (and .def_30 .def_51))
(define-fun .def_53 () Bool (and .def_32 .def_52))
(define-fun .def_54 () Bool (and .def_34 .def_53))
(define-fun .def_56 () Bool (and .def_54 .def_55))
(define-fun .def_57 () Bool (and .def_38 .def_56))
(define-fun .def_41 () Int (* (- 1) .y.10))
(define-fun .def_42 () Int (+ .def_8 .def_41))
(define-fun .def_43 () Bool (= .def_42 (- 1)))
(define-fun .def_58 () Bool (and .def_43 .def_57))
(define-fun .def_82 () Bool (or .def_58 .def_81))
(define-fun .def_45 () (Array Int Int) (store .def_36 .def_8 .def_10))
(define-fun .def_46 () Int (select .def_45 .def_14))
(define-fun .def_47 () Bool (= .y.14 .def_46))
(define-fun .def_25 () Bool (and .def_22 .def_24))
(define-fun .def_27 () Bool (and .def_25 .def_26))
(define-fun .def_29 () Bool (and .def_27 .def_28))
(define-fun .def_31 () Bool (and .def_29 .def_30))
(define-fun .def_33 () Bool (and .def_31 .def_32))
(define-fun .def_35 () Bool (and .def_33 .def_34))
(define-fun .def_39 () Bool (and .def_35 .def_38))
(define-fun .def_44 () Bool (and .def_39 .def_43))
(define-fun .def_48 () Bool (and .def_44 .def_47))
(define-fun .def_83 () Bool (or .def_48 .def_82))
(define-fun .def_85 () Bool (! (and .def_83 .def_84) :trans true))
(define-fun .def_93 () Bool (and .def_20 .def_28))
(define-fun .def_95 () Bool (! (not .def_93) :invar-property 0))
(assert true)
