(set-info :source |printed by MathSAT|)
(declare-fun M_4 () Int)
(declare-fun .y.13 () Int)
(declare-fun G_5 () Int)
(declare-fun .y.14 () Int)
(declare-fun .y.9 () Int)
(declare-fun .y.15 () Int)
(declare-fun N_4 () (Array Int Int))
(declare-fun .y.16 () (Array Int Int))
(declare-fun J_5 () Int)
(declare-fun .y.17 () Int)
(declare-fun .loc.10 () Bool)
(declare-fun .loc.18 () Bool)
(declare-fun .loc.11 () Bool)
(declare-fun .loc.19 () Bool)
(declare-fun .loc.12 () Bool)
(declare-fun .loc.20 () Bool)
(declare-fun H_5 () Int)
(declare-fun F_5 () Int)
(declare-fun O_3 () Int)
(declare-fun L_5 () Int)
(declare-fun K_5 () Int)
(define-fun .def_8 () Int (! M_4 :next |.y.13|))
(define-fun .def_10 () Int (! G_5 :next |.y.14|))
(define-fun .def_12 () Int (! .y.9 :next |.y.15|))
(define-fun .def_14 () (Array Int Int) (! N_4 :next |.y.16|))
(define-fun .def_16 () Int (! J_5 :next |.y.17|))
(define-fun .def_18 () Bool (! .loc.10 :next |.loc.18|))
(define-fun .def_20 () Bool (! .loc.11 :next |.loc.19|))
(define-fun .def_22 () Bool (! .loc.12 :next |.loc.20|))
(define-fun .def_40 () Int (! H_5 :next |.xtv.1|))
(define-fun .def_41 () Int (! F_5 :next |.xtv.2|))
(define-fun .def_87 () Int (! O_3 :next |.xtv.3|))
(define-fun .def_102 () Int (! L_5 :next |.xtv.4|))
(define-fun .def_119 () Int (! K_5 :next |.xtv.5|))
(define-fun .def_75 () Bool (not .def_18))
(define-fun .def_33 () Bool (not .def_20))
(define-fun .def_244 () Bool (and .def_33 .def_75))
(define-fun .def_242 () Bool (not .def_22))
(define-fun .def_245 () Bool (! (and .def_242 .def_244) :init true))
(define-fun .def_199 () Int (select .def_14 .def_41))
(define-fun .def_198 () Int (select .def_14 .def_87))
(define-fun .def_200 () Bool (= .def_198 .def_199))
(define-fun .def_229 () Bool (not .def_200))
(define-fun .def_135 () Int (* (- 16777216) .def_10))
(define-fun .def_142 () Int (* (- 1) .def_135))
(define-fun .def_143 () Real (* (/ 1 16777216) (to_real .def_142)))
(define-fun .def_144 () Int (to_int .def_143))
(define-fun .def_146 () Real (* (/ 1 2) (to_real .def_144)))
(define-fun .def_147 () Int (to_int .def_146))
(define-fun .def_213 () Bool (<= .def_147 .def_16))
(define-fun .def_214 () Bool (not .def_213))
(define-fun .def_126 () Int (* (- 1) .def_119))
(define-fun .def_184 () Int (+ .def_102 .def_126))
(define-fun .def_185 () Bool (= .def_184 (- 1)))
(define-fun .def_180 () Int (* (- 1) .def_40))
(define-fun .def_138 () Int (* 16777216 .def_10))
(define-fun .def_181 () Int (+ .def_138 .def_180))
(define-fun .def_182 () Bool (= .def_181 0))
(define-fun .def_130 () Int (* (- 1) .def_87))
(define-fun .def_176 () Int (+ .def_130 .def_102))
(define-fun .def_177 () Int (+ .def_8 .def_176))
(define-fun .def_178 () Bool (= .def_177 0))
(define-fun .def_53 () Int (* (- 1) .def_41))
(define-fun .def_171 () Int (+ .def_16 .def_53))
(define-fun .def_172 () Int (+ .def_8 .def_171))
(define-fun .def_173 () Bool (= .def_172 0))
(define-fun .def_179 () Bool (and .def_173 .def_178))
(define-fun .def_183 () Bool (and .def_179 .def_182))
(define-fun .def_186 () Bool (and .def_183 .def_185))
(define-fun .def_90 () Bool (<= .def_8 0))
(define-fun .def_92 () Bool (not .def_90))
(define-fun .def_187 () Bool (and .def_92 .def_186))
(define-fun .def_225 () Bool (and .def_187 .def_214))
(define-fun .def_67 () Bool (<= .def_41 0))
(define-fun .def_68 () Bool (not .def_67))
(define-fun .def_188 () Bool (or .def_68 .def_90))
(define-fun .def_226 () Bool (and .def_188 .def_225))
(define-fun .def_88 () Bool (<= .def_87 0))
(define-fun .def_89 () Bool (not .def_88))
(define-fun .def_91 () Bool (or .def_89 .def_90))
(define-fun .def_227 () Bool (and .def_91 .def_226))
(define-fun .def_191 () Real (* (/ 1 16777216) (to_real .def_40)))
(define-fun .def_192 () Int (to_int .def_191))
(define-fun .def_193 () Int (* (- 1) .def_192))
(define-fun .def_194 () Int (+ .def_119 .def_193))
(define-fun .def_195 () Int (+ .def_16 .def_194))
(define-fun .def_196 () Bool (= .def_195 0))
(define-fun .def_228 () Bool (and .def_196 .def_227))
(define-fun .def_230 () Bool (and .def_228 .def_229))
(define-fun .def_231 () Bool (and .def_18 .def_230))
(define-fun .def_232 () Bool (and .loc.20 .def_231))
(define-fun .def_96 () Bool (not .loc.18))
(define-fun .def_233 () Bool (and .def_96 .def_232))
(define-fun .def_25 () Bool (not .loc.19))
(define-fun .def_234 () Bool (and .def_25 .def_233))
(define-fun .def_219 () Bool (= .y.17 0))
(define-fun .def_216 () Bool (and .loc.18 .loc.19))
(define-fun .def_27 () Bool (not .loc.20))
(define-fun .def_217 () Bool (and .def_27 .def_216))
(define-fun .def_218 () Bool (and .def_75 .def_217))
(define-fun .def_220 () Bool (and .def_218 .def_219))
(define-fun .def_148 () Bool (<= .def_147 .def_12))
(define-fun .def_221 () Bool (and .def_148 .def_220))
(define-fun .def_29 () Bool (= .def_8 .y.13))
(define-fun .def_222 () Bool (and .def_29 .def_221))
(define-fun .def_31 () Bool (= .def_10 .y.14))
(define-fun .def_223 () Bool (and .def_31 .def_222))
(define-fun .def_158 () Bool (= .def_14 .y.16))
(define-fun .def_224 () Bool (and .def_158 .def_223))
(define-fun .def_235 () Bool (or .def_224 .def_234))
(define-fun .def_209 () Int (* (- 1) .y.17))
(define-fun .def_210 () Int (+ .def_16 .def_209))
(define-fun .def_211 () Bool (= .def_210 (- 1)))
(define-fun .def_189 () Bool (and .def_187 .def_188))
(define-fun .def_190 () Bool (and .def_91 .def_189))
(define-fun .def_197 () Bool (and .def_190 .def_196))
(define-fun .def_201 () Bool (and .def_197 .def_200))
(define-fun .def_202 () Bool (and .def_18 .def_201))
(define-fun .def_203 () Bool (and .loc.18 .def_202))
(define-fun .def_204 () Bool (and .loc.19 .def_203))
(define-fun .def_205 () Bool (and .def_27 .def_204))
(define-fun .def_206 () Bool (and .def_29 .def_205))
(define-fun .def_207 () Bool (and .def_31 .def_206))
(define-fun .def_208 () Bool (and .def_158 .def_207))
(define-fun .def_212 () Bool (and .def_208 .def_211))
(define-fun .def_215 () Bool (and .def_212 .def_214))
(define-fun .def_236 () Bool (or .def_215 .def_235))
(define-fun .def_237 () Bool (and .def_20 .def_236))
(define-fun .def_165 () Bool (= .y.14 .def_16))
(define-fun .def_163 () Bool (= .y.13 .def_10))
(define-fun .def_155 () Bool (and .def_18 .loc.19))
(define-fun .def_156 () Bool (and .def_96 .def_155))
(define-fun .def_157 () Bool (and .def_27 .def_156))
(define-fun .def_159 () Bool (and .def_157 .def_158))
(define-fun .def_160 () Bool (and .def_33 .def_159))
(define-fun .def_79 () Bool (= .y.15 0))
(define-fun .def_161 () Bool (and .def_79 .def_160))
(define-fun .def_59 () Int (* (- 16777216) .def_16))
(define-fun .def_60 () Int (* (- 1) .def_59))
(define-fun .def_62 () Real (* (/ 1 16777216) (to_real .def_60)))
(define-fun .def_63 () Int (to_int .def_62))
(define-fun .def_64 () Bool (<= .def_63 .def_12))
(define-fun .def_162 () Bool (and .def_64 .def_161))
(define-fun .def_164 () Bool (and .def_162 .def_163))
(define-fun .def_166 () Bool (and .def_164 .def_165))
(define-fun .def_238 () Bool (or .def_166 .def_237))
(define-fun .def_151 () Bool (<= .def_102 0))
(define-fun .def_152 () Bool (not .def_151))
(define-fun .def_153 () Bool (or .def_90 .def_152))
(define-fun .def_149 () Bool (not .def_148))
(define-fun .def_139 () Int (+ .def_138 .def_53))
(define-fun .def_140 () Bool (= .def_139 0))
(define-fun .def_131 () Int (+ .def_12 .def_130))
(define-fun .def_132 () Int (+ .def_8 .def_131))
(define-fun .def_133 () Bool (= .def_132 0))
(define-fun .def_127 () Int (+ .def_40 .def_126))
(define-fun .def_128 () Bool (= .def_127 1))
(define-fun .def_117 () Int (* (- 1) .def_102))
(define-fun .def_121 () Int (+ .def_117 .def_119))
(define-fun .def_122 () Int (+ .def_8 .def_121))
(define-fun .def_123 () Bool (= .def_122 0))
(define-fun .def_107 () Real (* (/ 1 16777216) (to_real .def_41)))
(define-fun .def_108 () Int (to_int .def_107))
(define-fun .def_109 () Int (* (- 1) .def_108))
(define-fun .def_112 () Int (+ .def_40 .def_109))
(define-fun .def_113 () Int (+ .def_12 .def_112))
(define-fun .def_114 () Bool (= .def_113 0))
(define-fun .def_103 () Int (select .def_14 .def_102))
(define-fun .def_104 () (Array Int Int) (store .def_14 .def_87 .def_103))
(define-fun .def_105 () Bool (= .y.16 .def_104))
(define-fun .def_93 () Bool (and .def_91 .def_92))
(define-fun .def_94 () Bool (and .def_20 .def_93))
(define-fun .def_95 () Bool (and .loc.19 .def_94))
(define-fun .def_97 () Bool (and .def_95 .def_96))
(define-fun .def_98 () Bool (and .def_27 .def_97))
(define-fun .def_99 () Bool (and .def_75 .def_98))
(define-fun .def_100 () Bool (and .def_29 .def_99))
(define-fun .def_101 () Bool (and .def_31 .def_100))
(define-fun .def_106 () Bool (and .def_101 .def_105))
(define-fun .def_115 () Bool (and .def_106 .def_114))
(define-fun .def_36 () Int (* (- 1) .y.15))
(define-fun .def_37 () Int (+ .def_12 .def_36))
(define-fun .def_38 () Bool (= .def_37 (- 1)))
(define-fun .def_116 () Bool (and .def_38 .def_115))
(define-fun .def_124 () Bool (and .def_116 .def_123))
(define-fun .def_129 () Bool (and .def_124 .def_128))
(define-fun .def_134 () Bool (and .def_129 .def_133))
(define-fun .def_141 () Bool (and .def_134 .def_140))
(define-fun .def_150 () Bool (and .def_141 .def_149))
(define-fun .def_154 () Bool (and .def_150 .def_153))
(define-fun .def_239 () Bool (or .def_154 .def_238))
(define-fun .def_84 () Bool (<= .y.14 0))
(define-fun .def_85 () Bool (not .def_84))
(define-fun .def_81 () Bool (<= .y.17 0))
(define-fun .def_82 () Bool (not .def_81))
(define-fun .def_73 () Bool (and .loc.18 .def_25))
(define-fun .def_74 () Bool (and .def_27 .def_73))
(define-fun .def_76 () Bool (and .def_74 .def_75))
(define-fun .def_77 () Bool (and .def_29 .def_76))
(define-fun .def_78 () Bool (and .def_33 .def_77))
(define-fun .def_80 () Bool (and .def_78 .def_79))
(define-fun .def_83 () Bool (and .def_80 .def_82))
(define-fun .def_86 () Bool (and .def_83 .def_85))
(define-fun .def_240 () Bool (or .def_86 .def_239))
(define-fun .def_71 () Bool (= .def_16 .y.17))
(define-fun .def_46 () Bool (<= .def_10 0))
(define-fun .def_69 () Bool (or .def_46 .def_68))
(define-fun .def_65 () Bool (not .def_64))
(define-fun .def_54 () Int (+ .def_12 .def_53))
(define-fun .def_55 () Int (+ .def_10 .def_54))
(define-fun .def_56 () Bool (= .def_55 0))
(define-fun .def_47 () Bool (not .def_46))
(define-fun .def_42 () (Array Int Int) (store .def_14 .def_41 .def_40))
(define-fun .def_43 () Bool (= .y.16 .def_42))
(define-fun .def_24 () Bool (and .def_18 .loc.18))
(define-fun .def_26 () Bool (and .def_24 .def_25))
(define-fun .def_28 () Bool (and .def_26 .def_27))
(define-fun .def_30 () Bool (and .def_28 .def_29))
(define-fun .def_32 () Bool (and .def_30 .def_31))
(define-fun .def_34 () Bool (and .def_32 .def_33))
(define-fun .def_39 () Bool (and .def_34 .def_38))
(define-fun .def_44 () Bool (and .def_39 .def_43))
(define-fun .def_48 () Bool (and .def_44 .def_47))
(define-fun .def_57 () Bool (and .def_48 .def_56))
(define-fun .def_66 () Bool (and .def_57 .def_65))
(define-fun .def_70 () Bool (and .def_66 .def_69))
(define-fun .def_72 () Bool (and .def_70 .def_71))
(define-fun .def_241 () Bool (or .def_72 .def_240))
(define-fun .def_243 () Bool (! (and .def_241 .def_242) :trans true))
(define-fun .def_246 () Bool (and .def_22 .def_244))
(define-fun .def_247 () Bool (! (not .def_246) :invar-property 0))
(assert true)
