(set-info :source |printed by MathSAT|)
(declare-fun A_1 () (Array Int Int))
(declare-fun .x.2 () (Array Int Int))
(declare-fun B_1 () (Array Int Int))
(declare-fun .x.3 () (Array Int Int))
(declare-fun C_1 () Int)
(declare-fun v_4 () Int)
(declare-fun D_0 () Int)
(declare-fun .x.4 () Int)
(declare-fun .loc.2 () Bool)
(declare-fun .loc.3 () Bool)
(declare-fun E () Int)
(declare-fun E_1 () Int)
(define-fun .def_8 () (Array Int Int) (! A_1 :next |.x.2|))
(define-fun .def_10 () (Array Int Int) (! B_1 :next |.x.3|))
(define-fun .def_12 () Int (! C_1 :next |v_4|))
(define-fun .def_14 () Int (! D_0 :next |.x.4|))
(define-fun .def_16 () Bool (! .loc.2 :next |.loc.3|))
(define-fun .def_29 () Int (! E :next |.xtv.1|))
(define-fun .def_66 () Int (! E_1 :next |.xpv.1|))
(define-fun .def_64 () Bool (= .def_12 0))
(define-fun .def_18 () Bool (not .def_16))
(define-fun .def_65 () Bool (! (and .def_18 .def_64) :init true))
(define-fun .def_62 () Bool (= .def_14 .x.4))
(define-fun .def_54 () Bool (= v_4 0))
(define-fun .def_19 () Bool (<= .def_14 .def_12))
(define-fun .def_55 () Bool (and .def_19 .def_54))
(define-fun .def_52 () Bool (= .def_8 .x.2))
(define-fun .def_56 () Bool (and .def_52 .def_55))
(define-fun .def_51 () Bool (= .def_10 .x.3))
(define-fun .def_57 () Bool (and .def_51 .def_56))
(define-fun .def_58 () Bool (and .def_18 .def_57))
(define-fun .def_45 () Int (select .def_8 .def_12))
(define-fun .def_47 () Int (+ .def_45 (- 1)))
(define-fun .def_48 () (Array Int Int) (store .def_8 .def_12 .def_47))
(define-fun .def_49 () Bool (= .x.2 .def_48))
(define-fun .def_39 () Int (select .def_10 .def_12))
(define-fun .def_41 () Int (+ .def_39 2))
(define-fun .def_42 () (Array Int Int) (store .def_10 .def_12 .def_41))
(define-fun .def_43 () Bool (= .x.3 .def_42))
(define-fun .def_20 () Bool (not .def_19))
(define-fun .def_36 () Bool (and .def_16 .def_20))
(define-fun .def_23 () Int (* (- 1) v_4))
(define-fun .def_24 () Int (+ .def_12 .def_23))
(define-fun .def_25 () Bool (= .def_24 (- 1)))
(define-fun .def_37 () Bool (and .def_25 .def_36))
(define-fun .def_44 () Bool (and .def_37 .def_43))
(define-fun .def_50 () Bool (and .def_44 .def_49))
(define-fun .def_59 () Bool (or .def_50 .def_58))
(define-fun .def_60 () Bool (and .loc.3 .def_59))
(define-fun .def_33 () (Array Int Int) (store .def_8 .def_12 .def_29))
(define-fun .def_34 () Bool (= .x.2 .def_33))
(define-fun .def_30 () (Array Int Int) (store .def_10 .def_12 .def_29))
(define-fun .def_31 () Bool (= .x.3 .def_30))
(define-fun .def_27 () Bool (not .loc.3))
(define-fun .def_21 () Bool (and .def_18 .def_20))
(define-fun .def_26 () Bool (and .def_21 .def_25))
(define-fun .def_28 () Bool (and .def_26 .def_27))
(define-fun .def_32 () Bool (and .def_28 .def_31))
(define-fun .def_35 () Bool (and .def_32 .def_34))
(define-fun .def_61 () Bool (or .def_35 .def_60))
(define-fun .def_63 () Bool (! (and .def_61 .def_62) :trans true))
(define-fun .def_73 () Bool (<= .def_66 0))
(define-fun .def_74 () Bool (not .def_73))
(define-fun .def_69 () Int (select .def_8 .def_66))
(define-fun .def_70 () Int (select .def_10 .def_66))
(define-fun .def_71 () Bool (<= .def_70 .def_69))
(define-fun .def_72 () Bool (not .def_71))
(define-fun .def_75 () Bool (and .def_72 .def_74))
(define-fun .def_76 () Bool (and .def_19 .def_75))
(define-fun .def_67 () Bool (<= .def_14 .def_66))
(define-fun .def_68 () Bool (not .def_67))
(define-fun .def_77 () Bool (and .def_68 .def_76))
(define-fun .def_78 () Bool (and .def_16 .def_77))
(define-fun .def_80 () Bool (! (not .def_78) :invar-property 0))
(assert true)
