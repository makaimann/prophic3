; chc-comp19-benchmarks/./lia-lin-arr/chc-lia-lin-arr-0040_000.smt2
(set-logic HORN)
...
(assert
  (forall ( (A (Array Int Int)) (B Int) (C Int) (D Int) (E (Array Int Int)) (F Int) ) 
    (=>
      (and
        (inv3 A B C)
        (let ((a!1 (= (store A B (+ (select A B) (* (- 1) D))) E)))
  (and a!1 (not (<= D B)) (not (<= C B)) (= F (+ 1 B))))
      )
      (inv3 E F C)
    )
  )
...
(assert
  (forall ( (A (Array Int Int)) (B Int) (C Int) (D Int) ) 
    (=>
      (and
        (inv4 A B C)
        (and (<= 0 (select A D)) (not (<= C D)) (not (<= D 0)) (>= B C))
      )
      false
    )
  )
