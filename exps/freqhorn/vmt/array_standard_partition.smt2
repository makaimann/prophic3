(set-info :source |printed by MathSAT|)
(declare-fun a_1 () (Array Int Int))
(declare-fun .y.8 () (Array Int Int))
(declare-fun b_1 () (Array Int Int))
(declare-fun .y.9 () (Array Int Int))
(declare-fun i_0 () Int)
(declare-fun .y.10 () Int)
(declare-fun N_1 () Int)
(declare-fun .y.11 () Int)
(declare-fun k_0 () Int)
(declare-fun .y.12 () Int)
(declare-fun .loc.7 () Bool)
(declare-fun .loc.13 () Bool)
(declare-fun i1_0 () Int)
(define-fun .def_8 () (Array Int Int) (! a_1 :next |.y.8|))
(define-fun .def_10 () (Array Int Int) (! b_1 :next |.y.9|))
(define-fun .def_12 () Int (! i_0 :next |.y.10|))
(define-fun .def_14 () Int (! N_1 :next |.y.11|))
(define-fun .def_16 () Int (! k_0 :next |.y.12|))
(define-fun .def_18 () Bool (! .loc.7 :next |.loc.13|))
(define-fun .def_45 () Int (! i1_0 :next |.xtv.1|))
(define-fun .def_60 () Bool (= .def_16 0))
(define-fun .def_59 () Bool (= .def_12 0))
(define-fun .def_61 () Bool (and .def_59 .def_60))
(define-fun .def_57 () Bool (! (not .def_18) :invar-property 0))
(define-fun .def_62 () Bool (! (and .def_57 .def_61) :init true))
(define-fun .def_51 () Int (select .def_10 .def_45))
(define-fun .def_52 () Bool (<= .def_51 0))
(define-fun .def_53 () Bool (not .def_52))
(define-fun .def_48 () Bool (<= .def_16 .def_45))
(define-fun .def_49 () Bool (not .def_48))
(define-fun .def_46 () Bool (<= 0 .def_45))
(define-fun .def_21 () Bool (<= .def_14 .def_12))
(define-fun .def_47 () Bool (and .def_21 .def_46))
(define-fun .def_50 () Bool (and .def_47 .def_49))
(define-fun .def_54 () Bool (and .def_50 .def_53))
(define-fun .def_55 () Bool (and .loc.13 .def_54))
(define-fun .def_41 () Int (* (- 1) .y.10))
(define-fun .def_42 () Int (+ .def_12 .def_41))
(define-fun .def_43 () Bool (= .def_42 (- 1)))
(define-fun .def_31 () Int (select .def_8 .def_12))
(define-fun .def_36 () (Array Int Int) (store .def_10 .def_16 .def_31))
(define-fun .def_32 () Bool (<= .def_31 0))
(define-fun .def_37 () (Array Int Int) (ite .def_32 .def_36 .def_10))
(define-fun .def_38 () Bool (= .y.9 .def_37))
(define-fun .def_29 () Int (+ .def_16 1))
(define-fun .def_33 () Int (ite .def_32 .def_29 .def_16))
(define-fun .def_34 () Bool (= .y.12 .def_33))
(define-fun .def_26 () Bool (= .def_8 .y.8))
(define-fun .def_24 () Bool (= .def_14 .y.11))
(define-fun .def_22 () Bool (not .def_21))
(define-fun .def_20 () Bool (not .loc.13))
(define-fun .def_23 () Bool (and .def_20 .def_22))
(define-fun .def_25 () Bool (and .def_23 .def_24))
(define-fun .def_27 () Bool (and .def_25 .def_26))
(define-fun .def_35 () Bool (and .def_27 .def_34))
(define-fun .def_39 () Bool (and .def_35 .def_38))
(define-fun .def_44 () Bool (and .def_39 .def_43))
(define-fun .def_56 () Bool (or .def_44 .def_55))
(define-fun .def_58 () Bool (! (and .def_56 .def_57) :trans true))
(assert true)
