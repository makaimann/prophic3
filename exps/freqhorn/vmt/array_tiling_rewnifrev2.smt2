(set-info :source |printed by MathSAT|)
(declare-fun a_1 () (Array Int Int))
(declare-fun .y.6 () (Array Int Int))
(declare-fun i_1 () Int)
(declare-fun .y.7 () Int)
(declare-fun S_1 () Int)
(declare-fun .y.8 () Int)
(declare-fun .loc.5 () Bool)
(declare-fun .loc.9 () Bool)
(declare-fun i1_0 () Int)
(define-fun .def_8 () (Array Int Int) (! a_1 :next |.y.6|))
(define-fun .def_10 () Int (! i_1 :next |.y.7|))
(define-fun .def_12 () Int (! S_1 :next |.y.8|))
(define-fun .def_14 () Bool (! .loc.5 :next |.loc.9|))
(define-fun .def_52 () Bool (<= .def_12 1))
(define-fun .def_53 () Bool (not .def_52))
(define-fun .def_49 () Int (* (- 1) .def_12))
(define-fun .def_50 () Int (+ .def_10 .def_49))
(define-fun .def_51 () Bool (= .def_50 (- 2)))
(define-fun .def_54 () Bool (and .def_51 .def_53))
(define-fun .def_46 () Bool (! (not .def_14) :invar-property 0))
(define-fun .def_55 () Bool (! (and .def_46 .def_54) :init true))
(define-fun .def_40 () Int (select .def_8 i1_0))
(define-fun .def_41 () Bool (<= i1_0 .def_40))
(define-fun .def_42 () Bool (not .def_41))
(define-fun .def_37 () Bool (<= .def_12 i1_0))
(define-fun .def_38 () Bool (not .def_37))
(define-fun .def_18 () Bool (<= 0 .def_10))
(define-fun .def_35 () Bool (not .def_18))
(define-fun .def_34 () Bool (<= 0 i1_0))
(define-fun .def_36 () Bool (and .def_34 .def_35))
(define-fun .def_39 () Bool (and .def_36 .def_38))
(define-fun .def_43 () Bool (and .def_39 .def_42))
(define-fun .def_44 () Bool (and .loc.9 .def_43))
(define-fun .def_31 () Bool (= .def_12 .y.8))
(define-fun .def_27 () Int (* (- 1) .y.7))
(define-fun .def_28 () Int (+ .def_10 .def_27))
(define-fun .def_29 () Bool (= .def_28 1))
(define-fun .def_21 () Int (+ .def_10 1))
(define-fun .def_22 () (Array Int Int) (store .def_8 .def_10 .def_10))
(define-fun .def_23 () (Array Int Int) (store .def_22 .def_21 .def_21))
(define-fun .def_24 () Bool (= .y.6 .def_23))
(define-fun .def_16 () Bool (not .loc.9))
(define-fun .def_19 () Bool (and .def_16 .def_18))
(define-fun .def_25 () Bool (and .def_19 .def_24))
(define-fun .def_30 () Bool (and .def_25 .def_29))
(define-fun .def_32 () Bool (and .def_30 .def_31))
(define-fun .def_45 () Bool (or .def_32 .def_44))
(define-fun .def_47 () Bool (! (and .def_45 .def_46) :trans true))
(assert true)
