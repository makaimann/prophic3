(set-info :source |printed by MathSAT|)
(declare-fun a_1 () (Array Int Int))
(declare-fun .y.7 () (Array Int Int))
(declare-fun i_0 () Int)
(declare-fun .y.8 () Int)
(declare-fun N_1 () Int)
(declare-fun .y.9 () Int)
(declare-fun t_1 () Int)
(declare-fun .y.10 () Int)
(declare-fun .loc.6 () Bool)
(declare-fun .loc.11 () Bool)
(define-fun .def_8 () (Array Int Int) (! a_1 :next |.y.7|))
(define-fun .def_10 () Int (! i_0 :next |.y.8|))
(define-fun .def_12 () Int (! N_1 :next |.y.9|))
(define-fun .def_14 () Int (! t_1 :next |.y.10|))
(define-fun .def_16 () Bool (! .loc.6 :next |.loc.11|))
(define-fun .def_50 () Bool (= .def_10 0))
(define-fun .def_47 () Bool (<= .def_14 0))
(define-fun .def_48 () Bool (not .def_47))
(define-fun .def_45 () Bool (<= .def_12 .def_14))
(define-fun .def_46 () Bool (not .def_45))
(define-fun .def_49 () Bool (and .def_46 .def_48))
(define-fun .def_51 () Bool (and .def_49 .def_50))
(define-fun .def_43 () Bool (! (not .def_16) :invar-property 0))
(define-fun .def_52 () Bool (! (and .def_43 .def_51) :init true))
(define-fun .def_37 () Int (select .def_8 .def_14))
(define-fun .def_38 () Bool (= .def_37 0))
(define-fun .def_39 () Bool (not .def_38))
(define-fun .def_19 () Bool (<= .def_12 .def_10))
(define-fun .def_40 () Bool (and .def_19 .def_39))
(define-fun .def_41 () Bool (and .loc.11 .def_40))
(define-fun .def_32 () (Array Int Int) (store .def_8 .def_10 0))
(define-fun .def_33 () Bool (= .def_10 .def_14))
(define-fun .def_34 () (Array Int Int) (ite .def_33 .def_32 .def_8))
(define-fun .def_35 () Bool (= .y.7 .def_34))
(define-fun .def_27 () Int (* (- 1) .y.8))
(define-fun .def_28 () Int (+ .def_10 .def_27))
(define-fun .def_29 () Bool (= .def_28 (- 1)))
(define-fun .def_24 () Bool (= .def_12 .y.9))
(define-fun .def_22 () Bool (= .def_14 .y.10))
(define-fun .def_20 () Bool (not .def_19))
(define-fun .def_18 () Bool (not .loc.11))
(define-fun .def_21 () Bool (and .def_18 .def_20))
(define-fun .def_23 () Bool (and .def_21 .def_22))
(define-fun .def_25 () Bool (and .def_23 .def_24))
(define-fun .def_30 () Bool (and .def_25 .def_29))
(define-fun .def_36 () Bool (and .def_30 .def_35))
(define-fun .def_42 () Bool (or .def_36 .def_41))
(define-fun .def_44 () Bool (! (and .def_42 .def_43) :trans true))
(assert true)
