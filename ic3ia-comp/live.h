...
class KLiveCounter {
public:
    KLiveCounter(const Options &opts, msat_env env);
    msat_term prop() const;
    void init(msat_term prop);
    void increment();
    TermMap get_substitution(size_t counter_value) const;
    size_t bound() const;
    void get_encoding(msat_term &init, msat_term &trans,
...
public:
    ...
    msat_term fairness() const { return fairness_; }
    ///< the single fairness condition (unless live_klive_only)
    
    void set_klive_bound(size_t k);
    ///< if k-liveness is enabled, increment the k-live bound for the next
    ///< encode() call

    const TermList &new_statevars() const { return newstatevars_; }
    ///< new state variables generated by the last encode() call

    const KLiveCounter &kliveness_counter() const { return klive_counter_; }
    size_t kliveness_bound() const { return klive_bound_val_; }
    
private:
    void encode_fairness();
    ...
    msat_term fairness_; ///< the single fairness constraint (except when
                         ///< opts_.live_klive_only is true)
    ...
    KLiveCounter klive_counter_; ///< k-liveness counter structure
    msat_term klive_bound_; ///< k-liveness counter bound
    size_t klive_bound_val_; ///< numeric value of the k-liveness bound
