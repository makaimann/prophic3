...
bool LTLEncoder::init() {
  if (ltl_syms_.empty()) {
    ...
    ltl_X_ = msat_declare_function(env_, "ltl.X", tp);
    ltl_F_ = msat_declare_function(env_, "ltl.F", tp);
    ltl_G_ = msat_declare_function(env_, "ltl.G", tp);
    tps.push_back(btp);
    tp = msat_get_function_type(env_, &tps[0], tps.size(), btp);
    ltl_U_ = msat_declare_function(env_, "ltl.U", tp);
    ltl_V_ = msat_declare_function(env_, "ltl.V", tp);
    ...
    auto visit = [](msat_env e, msat_term t, int pre,
      ...
      case MSAT_TAG_IFF:
        if (!pre) {
          ...
          data->cache[t] = msat_make_term(e, s, &data->args[0]);
        }
        return MSAT_VISIT_PROCESS;
      default:
        if (!pre) {
          data->args = {t};
          data->cache[t] = msat_make_term(e, data->ltl_X, &data->args[0]);
        }
...
void LTLEncoder::encode(const TermMap &statevars, msat_term ltl,
  ...
  if (is_special_case(ltl, justice)) {
    ...
    for (auto p : el2f_) {
      msat_term v = p.first;
      msat_term n = vp_.fresh_var(v, ".next");
      sv[v] = n;
      nel2el_[n] = v;
    }
  ...
  bool ok = ts.initialize(tssv, init, trans, justice[0], true);
  assert(ok);
...
msat_term LTLEncoder::normalize(msat_term ltl) {
  struct Data {
    ...
    Data(LTLEncoder *e, TermMap &c, TermList &a) : enc(e), cache(c), args(a) {}
  };
  ...
  auto visit = [](msat_env e, msat_term t, int pre,
    ...
    if (!pre) {
      ...
      if (d->enc->is_F(t)) {
        ...
        tt = msat_make_not(
            e, d->enc->make_U(msat_make_true(e), msat_make_not(e, d->args[0])));
      } else if (d->enc->is_V(t)) {
        tt = msat_make_not(e, d->enc->make_U(msat_make_not(e, d->args[0]),
...
void LTLEncoder::get_elementary_subformulas(msat_term neg_f) {
  struct Data {
    ...
    msat_term make_el(msat_term t) {
      return vp.fresh_var(std::string("{EL_") + (enc->is_U(t) ? "U_" : "X_") +
...
msat_term LTLEncoder::get_constraints(const TermMap &statevars, msat_term neg_f,
                                      TermList &justice) {
  ...
  struct Data {
    ...
    Data(LTLEncoder *e, const TermMap &st, TermMap &m, TermMap &s, TermList &j,
         TermList &a, std::vector<UntilRule> &r, std::vector<NextRule> &n)
        : enc(e), sv(st), f2el(m), sat(s), justice(j), args(a), until_rules(r),
          next_rules(n) {}

    msat_term next(msat_env e, msat_term t) {
      auto func = [&](msat_term t) -> msat_term {
        auto it = sv.find(t);
        if (it != sv.end()) {
  ...
  auto visit = [](msat_env e, msat_term t, int pre,
    ...
    if (!pre) {
      ...
      if (d->enc->is_X(t)) {
        ...
        msat_term el = d->f2el[t];
        msat_term f =
            msat_make_or(e, d->args[1], msat_make_and(e, d->args[0], el));
        d->sat[t] = f;
        msat_term n = d->next(e, f);
        d->trans.push_back(msat_make_eq(e, el, n));
        d->until_rules.push_back({t, el, d->args[0], d->args[1]});
        d->justice.push_back(msat_make_or(e, msat_make_not(e, f), d->args[1]));
      } else {
        msat_term tt = msat_make_term(e, msat_term_get_decl(t), &d->args[0]);
        d->sat[t] = tt;
      }
    }
    return MSAT_VISIT_PROCESS;
  ...
  Data d(this, statevars, f2el_, sat_, justice, args_, until_rules_,
         next_rules_);
  msat_visit_term(env_, neg_f, visit, &d);
...
const std::vector<LTLEncoder::UntilRule> &LTLEncoder::until_rules() const {
  return until_rules_;
}

const std::vector<LTLEncoder::NextRule> &LTLEncoder::next_rules() const {
  return next_rules_;
}
...
void LTLEncoder::make_single_justice(TermMap &sv, TermList &justice,
  ...
  for (auto j : justice) {
    auto l = vp_.fresh_var(std::string("{J") + std::to_string(msat_term_id(j)) +
                           "}");
    fair2hist_.emplace_back(std::make_pair(j, l));
    auto n = vp_.fresh_var(l, ".next");
    sv[l] = n;
    ...
    trans = msat_make_and(
        ...
        msat_make_eq(env_, n,
                     make_ite(env_, accept, msat_make_false(env_),
                              make_ite(env_, msat_make_and(env_, ok, j),
...
msat_term LTLEncoder::get_temporal_formula(msat_term lbl) {
  ...
  const auto subst = [&](msat_term v) -> msat_term {
