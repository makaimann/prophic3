 ...
 */
bool read_ts(const Options &opts,
             TransitionSystem &ts, LTLEncoder &ltl, TransitionSystem &tableau,
             TransitionSystem &product, TermList &preds)
{
    msat_term *terms;
    char **annots;
    size_t n;
    ...
    int err =
        msat_annotated_list_from_smtlib2_file(ts.get_env(), f.file, &n,
                                              &terms, &annots);
    ...
    if (ltl_prop) {
        ...
        ltl_prop = true;
        prop = msat_make_not(
            ts.get_env(), ltl.make_G(ltl.make_F(
